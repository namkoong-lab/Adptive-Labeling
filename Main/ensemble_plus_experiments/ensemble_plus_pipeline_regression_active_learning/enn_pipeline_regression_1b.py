...
def experiment(dataset_config: DatasetConfig, model_config: ModelConfig, train_config: TrainConfig, enn_config: ENNConfig, direct_tensor_files, Predictor, device, seed_training, if_print = 0):
    ...
    restore_model(ENN_model, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[21,100,71,18,67,78,44,28,101,96],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[21,100,71,18,67,78,44,28,101,96]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 21,100,71,18,67,78,44,28,101,96': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 21,100,71,18,67,78,44,28,101,96': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    restore_model(ENN_model, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[21,72,39,96,84,13,80,59,85,101],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[21,72,39,96,84,13,80,59,85,101]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.legend()
    wandb.log({'ENN initial training loss 21,72,39,96,84,13,80,59,85,101': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 21,72,39,96,84,13,80,59,85,101': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[21,72,39,14,75,96,84,13,109,88],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[21,72,39,14,75,96,84,13,109,88]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 21,72,39,14,75,96,84,13,109,88': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 21,72,39,14,75,96,84,13,109,88': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[21,75,100,80,96,84,95,87,28,42],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[21,75,100,80,96,84,95,87,28,42]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 21,75,100,80,96,84,95,87,28,42': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 21,75,100,80,96,84,95,87,28,42': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[25,10,22,70,102,67,78,44,18,28],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[25,10,22,70,102,67,78,44,18,28]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 25,10,22,70,102,67,78,44,18,28': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 25,10,22,70,102,67,78,44,18,28': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[25,10,22,70,102,96,21,100,101,71],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[25,10,22,70,102,96,21,100,101,71]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 25,10,22,70,102,96,21,100,101,71': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 25,10,22,70,102,96,21,100,101,71': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
