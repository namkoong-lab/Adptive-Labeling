...
def parameter_regularization_loss(model, initial_parameters, regularization_strength):
    ...
    for name, param in model.named_parameters():
        ...
        reg_loss += torch.sum((param - initial_param) ** 2)
...
def experiment(dataset_config: DatasetConfig, model_config: ModelConfig, train_config: TrainConfig, enn_config: ENNConfig, direct_tensor_files, Predictor, device, seed_training, if_print = 0):
    ...
    if dataset_config.direct_tensors_bool:
        ...
        dataset_train = TabularDataset(x = init_train_x, y = init_train_y)
        dataloader_train = DataLoader(dataset_train, batch_size=model_config.batch_size, sampler=BootstrappedSampler(dataset_train))
    ...
    for i in range(enn_config.n_ENN_iter):
        ...
        for (inputs, labels) in dataloader_train:   #check what is dim of inputs, labels, ENN_model(inputs,z)
            ...
            for j in range(enn_config.z_samples): 
                ...
                reg_loss = parameter_regularization_loss(ENN_model, initial_parameters, (enn_config.ENN_opt_weight_decay*init_train_x.size(0)/init_train_x.size(0)))/enn_config.z_samples
                loss= loss+reg_loss
                loss.backward()
                aeverage_loss += loss
    ...
    y_combined = torch.cat([init_train_y, pool_y], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, sampler=BootstrappedSampler(dataset_train_and_pool))
    ...
    for i in range(enn_config.n_ENN_iter):
        ...
        for (idx_batch, inputs, labels) in dataloader_train_and_pool:   #check what is dim of inputs, labels, ENN_model(inputs,z)
            ...
            for j in range(enn_config.z_samples): 
                ...
                loss = loss_fn_init(outputs, labels.unsqueeze(dim=1))/enn_config.z_samples
                reg_loss = parameter_regularization_loss(ENN_model, initial_parameters, (enn_config.ENN_opt_weight_decay*init_train_x.size(0)/x_combined.size(0)))/enn_config.z_samples
                loss= loss+reg_loss
                loss.backward()
                aeverage_loss += loss
            clip_grad_norm_(ENN_model.parameters(), max_norm=2.0)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss all': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior all': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)


    restore_model(ENN_model, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[1,46,45,53,31,48,24,20,41,17],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[1,46,45,53,31,48,24,20,41,17]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, sampler=BootstrappedSampler(dataset_train_and_pool))
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss one each': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior one each': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    restore_model(ENN_model, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[29,12,22,9,14,19,18,37,50,36],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[29,12,22,9,14,19,18,37,50,36]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, sampler=BootstrappedSampler(dataset_train_and_pool))
    ...
    plt.legend()
    wandb.log({'ENN initial training loss selected': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior selected': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)


    restore_model(ENN_model, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[1,43,46,29,45,20,0,41,28,17],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[1,43,46,29,45,20,0,41,28,17]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, sampler=BootstrappedSampler(dataset_train_and_pool))
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss two each end 5 clusters': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior two each end 5 clusters': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    restore_model(ENN_model, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[43,1,9,18,2,17,28,25,26,38],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[43,1,9,18,2,17,28,25,26,38]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, sampler=BootstrappedSampler(dataset_train_and_pool))
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss two end clusters': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior two end clusters': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
