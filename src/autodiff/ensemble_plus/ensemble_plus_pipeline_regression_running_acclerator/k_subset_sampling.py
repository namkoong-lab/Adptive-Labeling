...
class SubsetOperator(torch.nn.Module):
    def __init__(self, k, tau=1.0, hard=False):            # k is the number of samples we want, tau is the temperature parameter, hard:denotes if we want to implement the straight through implementation or the simple implementation
    ...
    def forward(self, scores):                                # scores take in log(weights) of each sample      # scores: Typical shape: [batch_size,n] or [batch_size,n,1]  where you want to choose k elements for n elements
        ...
        for i in range(self.k):
            khot_mask = torch.max(1.0 - onehot_approx, torch.tensor([EPSILON], device=scores.device))            # we can autodiff through this, there is no issue .
