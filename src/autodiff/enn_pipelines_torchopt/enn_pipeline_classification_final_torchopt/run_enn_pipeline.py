...
import polyadic_sampler_classification as polyadic_sampler
...
def main_run_func():
    with wandb.init(project=PROJECT_NAME, entity=ENTITY) as run:
        ...
        temp_recall = config.temp_recall
        ...
        if direct_tensors_bool:
            ...
            train_y_binary = train_y_binary.to(device)
            test_y_binary = test_y_binary.to(device)
            pool_y_binary = pool_y_binary.to(device)
        ...
        model_predictor = Naive_Network(output_size=1).to(device)
        ...
        wandb.log({"val_final_var_recal": var_recall})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project_name", type=str, help="WandB project name", default='adaptive_sampling_enn_classification')
