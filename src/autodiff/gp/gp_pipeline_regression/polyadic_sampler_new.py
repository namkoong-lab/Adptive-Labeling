  ...
  if if_logits:
     ...
     for i in range(1+(no_anchor_points-1)/2,no_anchor_points):
        if test_random:
           ...
           value = anchor_points_list_test_points[index]
           anchor_points_list_test_points = torch.cat((anchor_points_list_test_points[:index], anchor_points_list_test_points[index+1:]))
           test_sample_idx = torch.cat([test_sample_idx, torch.randint(value, value+1, (1,))])
        else:
           test_sample_idx = torch.cat([test_sample_idx, torch.randint(i, i+1, (1,))])   


  elif if_logits_only_pool:
     anchor_points_list_test_points = torch.arange(1,no_anchor_points)
     pool_sample_idx = torch.randint(0, 1, (no_pool_points,))
     for i in range(1, no_anchor_points):
       pool_sample_idx = torch.cat([pool_sample_idx, torch.randint(i, i+1, (no_pool_points/((no_anchor_points-1)*2),))])
     test_sample_idx = torch.randint(0, 1, (no_train_points/10,))
     for i in range(1,1+(no_anchor_points-1)/5):
        if test_random:
           index = torch.randint(0, len(anchor_points_list_test_points), (1,)).item()
           value = anchor_points_list_test_points[index]
           anchor_points_list_test_points = torch.cat((anchor_points_list_test_points[:index], anchor_points_list_test_points[index+1:]))
           test_sample_idx = torch.cat([test_sample_idx, torch.randint(i, i+1, (no_train_points/5,))])
        else:
            test_sample_idx = torch.cat([test_sample_idx, torch.randint(i, i+1, (no_train_points/5,))])
     for i in range(1+(no_anchor_points-1)/5,1+(no_anchor_points-1)*2/5):
        if test_random:
           index = torch.randint(0, len(anchor_points_list_test_points), (1,)).item()
           value = anchor_points_list_test_points[index]
           anchor_points_list_test_points = torch.cat((anchor_points_list_test_points[:index], anchor_points_list_test_points[index+1:]))
           test_sample_idx = torch.cat([test_sample_idx, torch.randint(value, value+1, (no_train_points/20,))])
        else:
           test_sample_idx = torch.cat([test_sample_idx, torch.randint(i, i+1, (no_train_points/20,))])
     for i in range(1+(no_anchor_points-1)*2/5,1+(no_anchor_points-1)*3/5):
        if test_random:
           index = torch.randint(0, len(anchor_points_list_test_points), (1,)).item()
           value = anchor_points_list_test_points[index]
           anchor_points_list_test_points = torch.cat((anchor_points_list_test_points[:index], anchor_points_list_test_points[index+1:]))
           test_sample_idx = torch.cat([test_sample_idx, torch.randint(value, value+1, (no_train_points/50,))])
        else:
           test_sample_idx = torch.cat([test_sample_idx, torch.randint(i, i+1, (no_train_points/50,))])  
     for i in range(1+(no_anchor_points-1)*3/5,1+(no_anchor_points-1)*5/5):
        if test_random:
           index = torch.randint(0, len(anchor_points_list_test_points), (1,)).item()
           value = anchor_points_list_test_points[index]
           anchor_points_list_test_points = torch.cat((anchor_points_list_test_points[:index], anchor_points_list_test_points[index+1:]))
