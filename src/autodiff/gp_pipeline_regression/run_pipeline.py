...
def main_run_func():
    with wandb.init(project=PROJECT_NAME, entity=ENTITY) as run:
        ...
        dataset_model_name = config.dataset_model_name
        ...
        if_logits = config.if_logits
        if_logits_only_pool = config.if_logits_only_pool
        plot_folder = config.plot_folder
        
        
        direct_tensors_bool = config.direct_tensors_bool
        ...
        access_to_true_pool_y = config.access_to_true_pool_y
        hyperparameter_tune = config.hyperparameter_tune
        ...
        if direct_tensors_bool:
            if dataset_model_name == "blr":
                ...
                if gp_model_dataset_generation=="use_default":
                    ...
                    dataset_model = CustomizableGPModel(dumi_train_x, dumi_train_y, dataset_mean_module, dataset_base_kernel, dataset_likelihood)
        ...
        dataset_cfg = gp_pipeline_regression.DatasetConfig(direct_tensors_bool, train_csv_name, test_csv_name, pool_csv_name, y_column)
        ...
        train_cfg = gp_pipeline_regression.TrainConfig(n_train_iter = n_train_iter, n_samples = n_samples) #temp_var_recall is the new variable added here
        gp_cfg = gp_pipeline_regression.GPConfig(length_scale=length_scale, output_scale= output_scale, noise_var = noise_var, parameter_tune_lr = paramater_tune_lr, parameter_tune_weight_decay = parameter_tune_weight_decay, paramater_tune_epochs = paramater_tune_epochs)

        model_predictor = ConstantValueNetwork(constant_value=0.0, output_size=1).to(device)
        model_predictor.eval()
        
        var_square_loss = gp_pipeline_regression.experiment(dataset_cfg, model_cfg, train_cfg, enn_cfg, direct_tensor_files, model_predictor, device, if_print = 1)
