...
import enn_pipeline_classification.old_pipeline_enn_classification as old_pipeline_enn_classification
...
dataset_cfg = old_pipeline_enn_classification.DatasetConfig(train_csv_name, test_csv_name, pool_csv_name, "EVENT_LABEL")
model_cfg = old_pipeline_enn_classification.ModelConfig(batch_size_train = 64, batch_size_test = 64, batch_size_query = 100, temp_k_subset = 0.1, hidden_sizes_weight_NN = [50,50], meta_opt_lr = 0.001, n_classes = 2, n_epoch = 10, init_train_lr = 0.001, init_train_weight_decay = 0.1, n_train_init = 20)
train_cfg = old_pipeline_enn_classification.TrainConfig(n_train_iter = 15, n_ENN_iter = 15, ENN_opt_lr = 0.001, temp_var_recall = 0.5, z_dim = 8, N_iter = 100) #temp_var_recall is the new variable added here
enn_cfg = old_pipeline_enn_classification.ENNConfig(basenet_hidden_sizes = [50,50],  exposed_layers = [False, True], z_dim = 8, learnable_epinet_hiddens = [15,15], hidden_sizes_prior = [5,5], seed_base = 2, seed_learnable_epinet = 1, seed_prior_epinet = 0, alpha = 0.1)
...
old_pipeline_enn_classification.experiment(dataset_cfg, model_cfg, train_cfg, enn_cfg, model_predictor, device, if_print = 1)
