...
def experiment(dataset_config: DatasetConfig, model_config: ModelConfig, train_config: TrainConfig, enn_config: ENNConfig, direct_tensor_files, Predictor, device, seed_training, if_print = 0):
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 0': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 0': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    plt.legend()
    wandb.log({'ENN initial training loss 0,1': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 0,1': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[0,1,3,7,4],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[0,1,3,7,4]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 0,1,3,7,4': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 0,1,3,7,4': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[8],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[8]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 8': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 8': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 8,9': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 8,9': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[8,9,15,11,12],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[8,9,15,11,12]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 8,9,15,11,12': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 8,9,15,11,12': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[0,8],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[0,8]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 0,8': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 0,8': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[0,1,3,9,15],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[0,1,3,9,15]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 0,1,3,9,15': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 0,1,3,9,15': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 1': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 1': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 3': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 3': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 7': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 7': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 9': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 9': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
    ...
    x_combined = torch.cat([init_train_x, pool_x[[15],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[15]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ...
    plt.title('ENN initial training loss')
    plt.legend()
    wandb.log({'ENN initial training loss 15': wandb.Image(fig_enn_training)})
    plt.close(fig_enn_training)
    ...
    if init_train_x.size(1) == 1:
        ...
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()-2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        plt.scatter(test_x.squeeze().cpu().numpy(),posterior_mean.detach().cpu().numpy()+2*posterior_std.detach().cpu().numpy(),alpha=0.2)
        wandb.log({'ENN initial posterior 15': wandb.Image(fig_enn_posterior)})
        plt.close(fig_enn_posterior)
