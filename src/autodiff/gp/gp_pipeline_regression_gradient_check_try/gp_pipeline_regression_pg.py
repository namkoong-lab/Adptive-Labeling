...
class GPConfig:
    ...
    output_scale: float
    noise_var: float
    parameter_tune_lr: float                 #no meaning for reinforce gradient 
    parameter_tune_weight_decay: float       #no meaning for reinforce gradient
    parameter_tune_nepochs: int              #no meaning for reinforce gradient
    stabilizing_constant: float             #no meaning for reinforce gradient
...
def policy_gradient_train(gp_model, init_train_x, init_train_y, pool_x, pool_y, test_x, test_y, model_config, train_config, gp_config, Predictor, policy):
    ...
    for episode in tqdm(range(train_config.n_train_iter)):
        ...
        for t in range(1):
            ...
            for j in range(train_config.G_samples):
                ...
                #print("torch.log(prob[batch_ind])",torch.log(prob[batch_ind]))
                #print("log_pr_preliminary",log_pr)
                for i in range(batch_size_query):
                    log_pr = log_pr- torch.log(1 - prob[batch_ind[:i]].sum())
                    #print("i",i)
                    #print("torch.log(1 - prob[batch_ind[:i]].sum())",torch.log(1 - prob[batch_ind[:i]].sum()))
                action = batch_ind

                mean, loss, done = env.step(action) # env step, uq update
                #loss=loss.detach()
                #print("log_pr_final:", log_pr)
                #print("loss:", loss)
                loss_temp.append(log_pr*loss)
                env.reset()

            avg_loss = torch.stack(loss_temp).mean()
            #print("policy_before_grad:", policy)

            optimizer.zero_grad()
            avg_loss.backward()
            #print("this is happening once for reinforce")
            grad_store_2 = policy.grad
            clip_grad_norm_([policy], max_norm=2.0)
            optimizer.step()
            #print("policy_after_grad:", policy)
    ...
    return loss_pool[-1], policy, grad_store_2





def experiment(dataset_config: DatasetConfig, model_config: ModelConfig, train_config: TrainConfig, gp_config: GPConfig, direct_tensor_files, Predictor, device, policy, if_print = 0):
    ...
    var_square_loss, policy, grad_store_2 = policy_gradient_train(gp_model, init_train_x, init_train_y, pool_x, pool_y, test_x, test_y, model_config, train_config, gp_config, Predictor, policy)
    
    return var_square_loss, policy, grad_store_2
