...
def experiment(dataset_config: DatasetConfig, model_config: ModelConfig, train_config: TrainConfig, enn_config: ENNConfig, direct_tensor_files, Predictor, device, seed_training, if_print = 0):
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device,"train")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "all_pool")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "0,1")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "2,3")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "4,5")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "6,7")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "8,9")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    restore_model(ENN_base, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[10,11],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[10,11]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ENN_base.train()
    loss_fn_init = nn.CrossEntropyLoss()
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "10,11")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    restore_model(ENN_base, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[12,13],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[12,13]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ENN_base.train()
    loss_fn_init = nn.CrossEntropyLoss()
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "12,13")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    restore_model(ENN_base, initial_parameters)
    x_combined = torch.cat([init_train_x, pool_x[[14,15],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[14,15]]], dim=0)
    dataset_train_and_pool = TabularDatasetPool(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=False)
    ENN_base.train()
    loss_fn_init = nn.CrossEntropyLoss()
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "14,15")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, init_train_x, -1, device, "15")
    meta_mean, meta_loss = var_recall_estimator(ENN_base, ENN_prior, dataloader_test, Predictor, device, model_config.temp_recall, enn_config.z_dim, train_config.n_samples, train_config.n_iter_noise, enn_config.alpha)
    recall_actual = Recall_True(dataloader_test, Predictor, device)
    #meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    #l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
