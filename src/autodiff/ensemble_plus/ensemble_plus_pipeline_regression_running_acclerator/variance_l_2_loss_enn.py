...
def l2_loss(test_x, test_y, Predictor):
...
def var_l2_loss_estimator(fnet, fnet_prior, test_x, Predictor, z_dim, alpha, stdev_noise):    #expects test_x = [N,D] and model to be a gpytorch model

    res  = torch.empty((0), dtype=torch.float32, device=test_x.device)


    for z_pool in range(z_dim): #i is iter in ENN z
        
        #z_pool = torch.randn(z_dim, device=device)     # sample z
        fnet_y = fnet(test_x, z_pool) + alpha * fnet_prior(test_x, z_pool)          #fnet()  #enn output
