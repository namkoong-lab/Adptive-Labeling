...
import gp_pipeline_regression_modified
...
def main_run_func():    
        ...
        dataset_cfg = gp_pipeline_regression_modified.DatasetConfig(direct_tensors_bool, csv_file_train, csv_file_test, csv_file_pool, y_column)
        model_cfg = gp_pipeline_regression_modified.ModelConfig(access_to_true_pool_y = access_to_true_pool_y, hyperparameter_tune = hyperparameter_tune, batch_size_query = batch_size_query, temp_k_subset = temp_k_subset, meta_opt_lr = meta_opt_lr, meta_opt_weight_decay = meta_opt_weight_decay)
        train_cfg = gp_pipeline_regression_modified.TrainConfig(n_train_iter = n_train_iter, n_samples = n_samples, G_samples=G_samples, learning_rate_PG = learning_rate_PG, weight_decay = weight_decay) #temp_var_recall is the new variable added here
        ...
        gp_cfg = gp_pipeline_regression_modified.GPConfig(length_scale=length_scale, output_scale= output_scale, noise_std = noise_std, mean_constant = mean_constant)
        ...
        var_square_loss = gp_pipeline_regression_modified.experiment(dataset_cfg, model_cfg, train_cfg, gp_cfg, direct_tensor_files, model_predictor, device, if_print = 1)
