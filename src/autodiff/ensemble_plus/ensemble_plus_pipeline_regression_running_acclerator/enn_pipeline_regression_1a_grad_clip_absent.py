...
def train(trained_parameters_ENN_base, initial_parameters_ENN_base, initial_parameters_ENN_prior, init_train_x, init_train_y, pool_x, pool_y, test_x, test_y, dataset_config, model_config, train_config, enn_config, NN_weights, meta_opt,initial_parameters_predictor, pool_sample_idx, device, seed_training):
  ...
  g_tensor = torch.arange(train_config.G_samples).unsqueeze(1)
  g_tensor_y = torch.arange(train_config.G_samples).unsqueeze(1)
  g_tensor_dataset = TabularDataset(x = g_tensor, y = g_tensor_y)
  g_tensor_dataloader = DataLoader(g_tensor_dataset, batch_size=1, shuffle=False)


  NN_weights = accelerator.prepare(NN_weights)
  meta_opt = accelerator.prepare(meta_opt)
  g_tensor_dataloader = accelerator.prepare(g_tensor_dataloader)
  ...
  for i in range(train_config.n_train_iter):    # Should we do this multiple times or not
    ...
    for g_x, g_y in g_tensor_dataloader:
        ...
        #meta_loss.backward()
        aeverage_meta_loss += meta_loss
        wandb.log({"epoch+g_samples": i+g_x[0], "time_taken_per_g":intermediate_time_2-intermediate_time_1, "meta_loss": meta_loss.item(), "meta_mean": meta_mean.item()})
