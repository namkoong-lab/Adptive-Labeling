...
def experiment(dataset_config: DatasetConfig, model_config: ModelConfig, train_config: TrainConfig, enn_config: ENNConfig, direct_tensor_files, Predictor, device, seed_training, if_print = 0):
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='all_pool')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[0,1],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[0,1]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='0,1')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[2,3],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[2,3]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='2,3')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[2,4],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[2,4]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='2,4')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[4,5],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[4,5]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='4,5')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[6,7],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[6,7]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='6,7')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[8,9],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[8,9]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='8,9')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[7,9],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[7,9]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='7,9')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[7],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[7]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='7')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[9],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[9]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='9')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[9,5],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[9,5]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='9,5')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
    ...
    x_combined = torch.cat([init_train_x, pool_x[[7,3],:]], dim=0)
    y_combined = torch.cat([init_train_y, pool_y[[7,3]]], dim=0)
    dataset_train_and_pool = TabularDataset(x=x_combined, y=y_combined)
    dataloader_train_and_pool = DataLoader(dataset_train_and_pool, batch_size=train_config.batch_size, shuffle=dataset_config.shuffle)
    ...
    plot_ENN_training_posterior(ENN_base, ENN_prior, train_config, enn_config, enn_loss_list, test_x, test_y, x_combined, 0, device, label_plot='7,3')
    meta_mean, meta_loss = var_l2_loss_estimator(ENN_base, ENN_prior, test_x, Predictor, device, enn_config.z_dim, enn_config.alpha, enn_config.stdev_noise)
    l_2_loss_actual = l2_loss(test_x, test_y, Predictor, None)
    wandb.log({"var_square_loss": meta_loss.item(), "mean_square_loss": meta_mean.item(), "l_2_loss_actual": l_2_loss_actual.item()})
...
def train(ENN_base_new, ENN_prior, initial_parameters, init_train_x, init_train_y, pool_x, pool_y, test_x, test_y, device, dataset_config, model_config, train_config, enn_config, NN_weights, meta_opt, SubsetOperatorthis, Predictor, pool_sample_idx, if_print = 0):
  ...
  for i in range(train_config.n_train_iter):    # Should we do this multiple times or not
    ...
    if model_config.access_to_true_pool_y:
        ...
        pool_y_dumi = (ENN_base_new(pool_x, random_integer) + enn_config.alpha * ENN_prior(pool_x,random_integer)).squeeze().detach()    # assuming this can be handled by the GPUs otherwise put it in batches
    ...
    for g in range(train_config.G_samples):
        ...
        meta_loss.backward()
        #print("NN_weights.grad:", NN_weights.grad)
        aeverage_meta_loss += meta_loss
        wandb.log({"epoch+g_samples": i+g, "time_taken_per_g":intermediate_time_2-intermediate_time_1, "meta_loss": meta_loss.item(), "meta_mean": meta_mean.item()})
    

    #grad_norm_here = clip_grad_norm_([NN_weights], max_norm=2.0)
