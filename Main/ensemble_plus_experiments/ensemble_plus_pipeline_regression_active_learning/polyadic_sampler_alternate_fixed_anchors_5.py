...
def x_sampler(no_train_points, no_test_points, no_pool_points, no_anchor_points=3, input_dim = 1, stdev_scale = 0.2, stdev_pool_scale = 0.5, scaling_factor=None, scale_by_input_dim = True, logits = None, if_logits= False, if_logits_only_pool= False):
  ...
  if no_anchor_points>5:
    ...
    anchor_x = torch.tensor([[0.0],[1.0],[1.0],[-2.0],[2.0]])
  ...
  pool_sample_idx = torch.randint(0, 1, (2,))
  for i in range(1, no_anchor_points):
    pool_sample_idx = torch.cat([pool_sample_idx, torch.randint(i, i+1, (2,))]) 
  test_sample_idx = torch.randint(0, 1, (50,))
  for i in range(1, 3):
    test_sample_idx = torch.cat([test_sample_idx, torch.randint(i, i+1, (100,))]) 
  for i in range(3, 5):
