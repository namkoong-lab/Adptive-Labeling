#code https://github.com/swyoon/pytorch-minimal-gaussian-process/blob/main/binary_laplace_gpc.py
...
class BinaryLaplaceGPC(nn.Module):
    ...
    def fit(self, X, y):
        ...
        while True:
            ...
            sqrt_W = W.sqrt()
            L = torch.linalg.cholesky(torch.eye(N) + sqrt_W.mm(K.mm(sqrt_W)))
...
##code: https://github.com/swyoon/pytorch-minimal-gaussian-process/blob/main/demo_binary_gpc.ipynb
def GP_prediction_classification(train_x,train_y,test_x):
    ...
    upper = [min(max(0,_),1) for _ in upper]
    lower = [min(max(0,_),1)  for _ in lower]
 



    return {'lower':lower,'upper':upper,'mean':mu}