...
class TabularDataset(Dataset):
    def __init__(self, device, csv_file, y_column):
        """
        Args:
        ...
        self.x = torch.tensor(self.data_frame.drop(self.y_column, axis=1).values, dtype=torch.float32).to(device)
        #x = torch.tensor(self.data_frame.iloc[:, :-1].values, dtype=torch.float32)
        self.y = torch.tensor(self.data_frame[self.y_column].values, dtype=torch.float32).to(device)
